...
class SqlRequest:
    def __init__(self):
        ...
        self.__detailAttr = ( 'inventory_id', 'inventory_name', 'quantity', 'comment_inventory')
    ...
    def search_customer_id(self, user_id):
        if user_id is None:
            return False
        try:
            st = ' select x.user_name, x.user_addr, x.user_tel, x.user_privilege, x.user_id, y.customer_name, y.customer_email '
    ...
    def _parse_payment_status(self, dic_list):
        ...
        for dic in dic_list:
            if 'payment_status' in dic:
                dic['payment_status'] = self.__payment_status_dict[int(dic['payment_status'])]
        return dic_list
